...
impl<S, B> SendStream<S, B> {
    ...
    /// Reset the send stream with an application error code.
    pub fn reset_with_error_code(&mut self, error_code: u64) -> Result<(), StreamErrorIncoming>
    where
        S: quic::SendStream<B>,
        B: Buf,
    {
        <BufRecvStream<S, B> as quic::SendStream<B>>::reset_with_error_code(
            &mut self.stream,
            error_code,
        )
    }

    /// Abort the send stream with an application error code.
    pub fn abort(&mut self, error_code: u64) -> Result<(), StreamErrorIncoming>
    where
        S: quic::SendStream<B>,
        B: Buf,
    {
        <BufRecvStream<S, B> as quic::SendStream<B>>::abort(&mut self.stream, error_code)
    }

    /// Set the QUIC send priority for this stream, when supported by the transport.
    pub fn set_priority(&self, priority: i32) -> Result<(), StreamErrorIncoming>
    where
        S: quic::SendStream<B>,
        B: Buf,
    {
        <BufRecvStream<S, B> as quic::SendStream<B>>::set_priority(&self.stream, priority)
    }

    /// Return the QUIC send priority for this stream, when supported by the transport.
    pub fn priority(&self) -> Result<Option<i32>, StreamErrorIncoming>
    where
        S: quic::SendStream<B>,
        B: Buf,
    {
        <BufRecvStream<S, B> as quic::SendStream<B>>::priority(&self.stream)
    }
}
...
impl<S, B> BidiStream<S, B> {
    ...
    /// Reset the send side with an application error code.
    pub fn reset_with_error_code(&mut self, error_code: u64) -> Result<(), StreamErrorIncoming>
    where
        S: quic::SendStream<B>,
        B: Buf,
    ...
    /// Abort the send side with an application error code.
    pub fn abort(&mut self, error_code: u64) -> Result<(), StreamErrorIncoming>
    where
        S: quic::SendStream<B>,
        B: Buf,
