...
pub trait SendStream<B: Buf> {
    ...
    /// Send a QUIC reset code and report whether it was accepted locally.
    fn reset_with_error_code(&mut self, reset_code: u64) -> Result<(), StreamErrorIncoming> {
        self.reset(reset_code);
        Ok(())
    }

    /// Abort this send stream with a QUIC reset code.
    fn abort(&mut self, reset_code: u64) -> Result<(), StreamErrorIncoming> {
        self.reset_with_error_code(reset_code)
    }

    /// Set the send priority for implementations that expose stream priority.
    fn set_priority(&self, _priority: i32) -> Result<(), StreamErrorIncoming> {
        Ok(())
    }

    /// Return the send priority when the transport exposes it.
    fn priority(&self) -> Result<Option<i32>, StreamErrorIncoming> {
        Ok(None)
    }
